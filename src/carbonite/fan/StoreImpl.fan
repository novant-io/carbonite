...
//////////////////////////////////////////////////////////////////////////
  ...
  ** Return result from select join sql statement.
  virtual CRec[] _selectJoin(CTable table, Str cols, CTable join, Str joinCol, [Str:Obj]? where := null)
  {
    sql := SqlExpr.selectJoin(table.name, cols, join.name, joinCol, where)
    // TODO FIXIT: fix sql to go directly -> CRec and nuke Row type
    return onLockExec |conn| {
      return _exec(sql.expr, sql.params).map |row| { makeJoinRec(table, join, row) }
    }
  }
